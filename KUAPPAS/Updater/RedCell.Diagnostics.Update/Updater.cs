...
namespace RedCell.Diagnostics.Update
    //namespace RedCell.Diagnostics.Update
//namespace ASETWindClient.Update
{
    ...
    public  Process Check (object state)
        ...
        
        {
           // Log.Write("Check starting.");
            ...
            if (!http.Success)
            {
                ...
                MessageBox.Show(http.Response.StatusDescription);
                return null;
                //return;
            }
        ...
        private Process Update()
       {
           try
           {
               ...
               if (Directory.Exists(WorkPath))
               {
                 ...
                 //  MessageBox.Show("WARNING: Work directory already exists." + WorkPath);
                   try { Directory.Delete(WorkPath, true); }
                   catch (IOException)
                   {
                       Log.Write("Cannot delete open directory '{0}'.", WorkPath);
                  //     MessageBox.Show("Cannot delete open directory '{0}'.", WorkPath);
                       return null;
                      // return;
                   }
               }

        //       MessageBox.Show("creae directory {0}", WorkPath);
           ...
           //        MessageBox.Show("Fetching '{0}'.", update);
                 ...
                 //      MessageBox.Show("Fetch failed '{0}'.", url);
                       return null;
                       //return;
                   }
