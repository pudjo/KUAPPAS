...
namespace KUAPPAS
{
    public partial class frmRekening90 : ChildForm
    {
        private bool m_bNew;

        public frmRekening90()
        {
            InitializeComponent();
        }
        ...
        private void treeRekening1_Changed(Rekening rek)
        {
            if (rek != null)
            {
                ...
                if (rek.Root == 6)
                {
                    cmdTambahAnak.Enabled = false;
                }
                else
                {
                    cmdTambahAnak.Enabled = true;
                }
        ...
        private void cmdSimpan_Click(object sender, EventArgs e)
        {
            ...
            oRek.ID = ctrlKodeRekening.GetID(m_bNew);
        ...
        private void cmdTambahAnak_Click(object sender, EventArgs e)
        {
            ...
            if (oRekeningParent.Root == 6)
            {
                MessageBox.Show("Rekening ini sudah tidak bisa ditambah rincian lagi");
                return;
            }
            else
            {
        ...
        {
            RekeningLogic oLogic = new RekeningLogic(GlobalVar.TahunAnggaran, RekeningLogic.E_REKENING_TYPE.REKENING_13);
            if (oLogic.PerbaikiRoot() == true)
            {
                MessageBox.Show("Perbaiki root selesai");
            }
            else
            {
                MessageBox.Show("Perbaiki root Gagal");
            }
        }
    }
}
